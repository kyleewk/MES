               ...
               <div class="row">
                   <div class="col-md-12">
                       <div class="white-box">
                           ...
                           <a class="btn btn-info" href="${pageContext.request.contextPath}/productLine/toadd"><i class="fa fa fa-plus"></i> <span>添加</span></a>
                           <form class="search1" method="post"
                                 action="${pageContext.request.contextPath}/productLine/list"
                           >
                               <label>
                                   部门编号：
                                   <input name="id">
                            ...
                            <div class="table-responsive">
                             <table id="example" class="display table">
                                 ...
                                 <c:forEach items="${pageInfo.list}" var="o">
                                     <tr>
                                         <td>${o.id}</td>
                                         <td>${o.productdeparment}</td>
                                         <td>${o.productname}</td>
                                         <td>${o.producttype}</td>
                                         <td>${o.productorder}</td>
                                         <td>${o.peopletotal}</td>
                                         <td>${o.outputplan}</td>
                                         <td><fmt:formatDate value="${o.comlpetiondate}" pattern="yyyy/MM/dd HH:mm:ss"/> </td>
                                     </tr>
                                 </c:forEach>
                                 </tbody>
                                   </table>  
                           ...
                           <div class="page1" >
                               <c:if test="${pageInfo.hasPreviousPage}">
                                   <a class="btn btn-white" role="button" href="${pageContext.request.contextPath}/productLine/list?page=${pageInfo.prePage}">上一页</a>
                               </c:if>
                               <c:forEach items="${pageInfo.navigatepageNums}" var="n">
                                   <a href="${pageContext.request.contextPath}/productLine/list?page=${n}"><button type="button" class="btn btn-white" >${n}</button></a>
                               </c:forEach>
                               <c:if test="${pageInfo.hasNextPage}">
                                   <a class="btn btn-white" role="button" href="${pageContext.request.contextPath}/productLine/list?page=${pageInfo.nextPage}">下一页</a>
                               </c:if>
                           </div>
                       </div>
                   </div>

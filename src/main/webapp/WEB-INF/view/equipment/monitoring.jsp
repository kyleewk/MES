               ...
               <div class="row">
                   <div class="col-md-12">
                       <div class="white-box">
                           ...
                           <a class="btn btn-info" href="${pageContext.request.contextPath}/monitoring/toadd"><i class="fa fa fa-plus"></i> <span>添加</span></a>
                           <form class="search1" method="post"
                                 action="${pageContext.request.contextPath}/monitoring/list"
                           >
                               <label>
                                   监控序号：
                                   <input name="id">
                             ...
                             <table id="example" class="display table">
                                 ...
                                 <c:forEach items="${pageInfo.list}" var="o">
                                     <tr>
                                         ...
                                         <td>${o.standard}</td>
                                         <td>${o.record}</td>
                                         <td><fmt:formatDate value="${o.recorddate}" pattern="yyyy/MM/dd HH-mm-ss"/></td>
                                         <td>
                                             <a href="${pageContext.request.contextPath}/monitoring/toscan?id=${o.id}">删除</a>
                                         </td>
                                     </tr>
                                 </c:forEach>
                                 </tbody>
                           ...
                           <div class="page1" >
                               <c:if test="${pageInfo.hasPreviousPage}">
                                   <a class="btn btn-white" role="button" href="${pageContext.request.contextPath}/monitoring/list?page=${pageInfo.prePage}">上一页</a>
                               </c:if>
                               <c:forEach items="${pageInfo.navigatepageNums}" var="n">
                                   <a href="${pageContext.request.contextPath}/monitoring/list?page=${n}"><button type="button" class="btn btn-white" >${n}</button></a>
                               </c:forEach>
                               <c:if test="${pageInfo.hasNextPage}">
                                   <a class="btn btn-white" role="button" href="${pageContext.request.contextPath}/monitoring/list?page=${pageInfo.nextPage}">下一页</a>
                               </c:if>
                           </div>
                       </div>
                   </div>
